...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    load_weight: str = field(default=None, metadata={"help": 'Load a pretrained model from a checkpoint'})
...
class DataTrainingArguments:
    ...
    dataset_config_name: Optional[str] = field(
        default="", metadata={"help": "The configuration name of the dataset to use (via the datasets library)."}
    )
...
def main():
    ...
    if training_args.load_weight is not None:
        logger.info(f"loading weight from {training_args.load_weight}")
        model.load_state_dict(torch.load(training_args.load_weight))
    ...
    if training_args.do_train:
        ...
        # save model ckpt with torch.save
        # torch.save(trainer.model.state_dict(), os.path.join(training_args.output_dir, "model.ckpt"))
